...
const App = () => {
  ...
  useEffect(() => {
    const check = async () => {
      try {
        ...
        if (res.ok) { setBackendConnected(true); setBackendStatus('connected'); }
      } catch { setBackendConnected(false); setBackendStatus('offline — demo mode'); }
    };
    check();
  ...
  useEffect(() => {
    if (isRunning && demoMode) {
      intervalRef.current = setInterval(() => {
        setTick((t) => {
          const nt = t + 1;
          setSimState(generateDemoState(nt));
          return nt;
        });
      }, tickInterval);
    ...
    if (isRunning && !demoMode && simId) {
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (status.status === 'completed') { setIsRunning(false); setBackendStatus('complete'); }
          setTick(status.ticks_completed || tick);
          setSimState({ agentCount: status.summary?.ticks || 0, metrics: status.summary });
  ...
  const handleSelectScenario = useCallback(async (scenarioId) => {
    setScenario(scenarioId); setTick(0); setIsRunning(false);
    clearInterval(intervalRef.current); clearInterval(pollRef.current);
    if (backendConnected) {
      try {
        const result = await startSimulation(scenarioId);
        setSimId(result.sim_id); setDemoMode(false);
        setSimState({ agentCount: 0, metrics: {} });
        setIsRunning(true); setBackendStatus('running...'); return;
      } catch (e) { console.warn('Backend failed, demo mode:', e); }
    }
    setDemoMode(true); setSimState(generateDemoState(0));
  }, [backendConnected]);

  const metrics = simState?.metrics || (demoMode && isRunning ? {
  ...
  const handlePlay = () => {
    if (!simState && !scenario) { setSimState(generateDemoState(0)); setDemoMode(true); }
    setIsRunning(true);
  };
        ...
        {!sidebarCollapsed && (
          <aside className="sidebar">
            <div className="sidebar-header">
              <h3>📂 Scenarios</h3>
              <button className="icon-btn" onClick={() => setSidebarCollapsed(true)}>◀</button>
            </div>
            <ScenarioSelector selected={scenario} onSelect={handleSelectScenario} />
          </aside>
        )}
        {sidebarCollapsed && (
          <button className="expand-btn" onClick={() => setSidebarCollapsed(false)}>📂</button>
        )}

        <main className="main-content">
          <ControlPanel
            isRunning={isRunning} speed={speed}
            onPlay={handlePlay} onPause={() => setIsRunning(false)}
            onStep={() => { const t = tick + 1; setTick(t); setSimState(generateDemoState(t)); }}
            onReset={() => { setIsRunning(false); setTick(0); setSimId(null); setSimState(generateDemoState(0)); setDemoMode(true); }}
          ...
          <div className="content-grid">
            {activeView === '2d' ? (
              ...
              <World3D state={simState} timeOfDay={timeOfDay} />
            )}
